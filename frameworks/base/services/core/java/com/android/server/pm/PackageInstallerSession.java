...
public class PackageInstallerSession extends IPackageInstallerSession.Stub {
    ...
    private void computeProgressLocked(boolean forcePublish) {
        if (!isIncrementalInstallation() || !isCommitted()) {//普通安装模式
            //总进度 = min(客户端进度 × 0.8, 0.8) + min(内部进度 × 0.2, 0.2)
            // 客户端进度 (Client Progress)
            // mClientProgress  // 由 setClientProgress/addClientProgress 设置
            //安装器应用（如应用商店）/下载进度、解压进度等
            mProgress = MathUtils.constrain(mClientProgress * 0.8f, 0f, 0.8f)
                    // 内部进度 (Internal Progress)
                    // mInternalProgress  // 由 PMS 内部更新
                    // PackageManagerService / dex优化、签名验证、安装完成等
            ...
            //增量安装进度 (Incremental Progress) / mIncrementalProgress  // 由 DataLoader 更新
