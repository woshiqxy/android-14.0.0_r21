...
final class InitAppsHelper {
     ...
     * 初始化所有系统应用的核心方法，它在系统启动时被调用，负责扫描、解析和准备所有预置的系统应用
     * 锁保护机制
     * mInstallLock	安装操作	防止并发安装干扰系统扫描
     * mPm.mLock	PMS全局状态	保护mPackages等核心数据结构
     *
     * 系统扫描是最基础的操作，必须保证原子性
     * 防止扫描过程中有其他安装操作修改系统状态
     * 避免死锁：先获取mInstallLock再获取mLock
     */
    @GuardedBy({"mPm.mInstallLock", "mPm.mLock"})
    public OverlayConfig initSystemApps(PackageParser2 packageParser,
        ...
        //系统目录扫描
        //目录	内容	优先级
        ///system/priv-app	特权系统应用	最高
        ///system/app	普通系统应用	高
        ///vendor/app	厂商定制应用	中
        ///oem/app	OEM定制应用	中
        ///product/app	产品分区应用	中
        ///system_ext/app	系统扩展应用	高
     ...
     * 真正执行系统目录扫描的核心。它按照特定的优先级顺序，扫描所有系统分区中的应用目录
     */
    @GuardedBy({"mPm.mInstallLock", "mPm.mLock"})
    private void scanSystemDirs(PackageParser2 packageParser, ExecutorService executorService) {
        ...
        //mDirsToScanAsSystem 是一个 ScanPartition 列表，定义了所有需要扫描的系统分区：
    ...
    /**
     * scanDirTracedLI 是整个系统应用扫描链的最后一环，负责实际处理单个目录：
     * @param scanDir
     * @param parseFlags
     * @param scanFlags
     * @param packageParser
