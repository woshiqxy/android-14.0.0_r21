...
final class InstallPackageHelper {
     ...
     * 负责将经过协调（Reconciliation）的扫描结果最终提交到系统中
     * ReconciledPackage (协调结果)
     *     ↓
     * InstallRequest (安装请求)
     *     ├── ParsedPackage (解析的包)
     *     ├── PackageSetting (包设置)
    ...
    public AndroidPackage commitReconciledScanResultLocked(
        ...
        //android 包是框架本身
        //强制设置版本号为当前 SDK 版本（如 Android 14 对应 34）
        //确保系统框架版本与系统一致
        ...
        //SharedUser 机制：
        //多个应用共享同一个 UID
        //当应用的 SharedUser 改变时，需要从旧的 SharedUser 中移除
        //如果 SharedUser 没有其他应用，则清理该 SharedUser
        ...
        //两种更新模式：
        //复制模式：复用原有的 PackageSetting，只更新变化的部分
        //替换模式：使用新的 PackageSetting，并处理重命名包
        ...
        //InstallSource 包含：
        //mInitiatingPackageName：发起安装的包名
        //mInstallerPackageName：实际执行安装的包名
        //mUpdateOwnerPackageName：更新所有者包名
        //mInitiatingPackageSignatures：发起者的签名
     ...
     *     ├── 特殊包处理 (android/custom resolver)
     *     ├── 共享库更新 (commitSharedLibraryChanges)
     *     ├── 依赖应用处理 (kill dependent apps)
     *     ├── 写入Settings (mSettings.insertPackageSettingLPw)
     *     ├── 写入mPackages (mPackages.put)
     *     ├── ComponentResolver更新 (添加四大组件)
     *     ├── AppsFilter更新 (权限过滤)
     *     ├── DomainVerification更新 (域名验证)
     *     ├── Instrumentation注册
     *     ├── ProtectedBroadcasts添加
     *     └── PermissionManager更新
     */
    private void commitPackageSettings(@NonNull AndroidPackage pkg,
        ...
        if (mPm.mCustomResolverComponentName != null
            ...
            //设置自定义的默认Resolver（如华为、小米可能替换默认选择器）
            //OEM厂商可以替换Android默认的"完成操作"选择界面
        ...
        synchronized (mPm.mLock) {
            ...
            //关键数据结构：
            //mSettings：持久化存储（写入packages.xml）
            //mPackages：内存缓存，快速查找
            //mApexManager：APEX模块管理
            ...
            //ComponentResolver更新
            //ComponentResolver职责：
            //管理四大组件（Activity、Service、Receiver、Provider）
            //建立组件名称到组件信息的映射
            //处理Intent过滤（Intent Filter）
    ...
    /**
     * 主要作用是为指定的用户（userId）安装一个已经存在于设备上的应用程序包。
     * 换句话说，当某个应用已经被安装到设备中（例如系统应用或已被其他用户安装的应用），
     * 但当前目标用户尚未安装（即该用户的应用列表中未出现），可以通过此方法为该用户“激活”该应用，使其对目标用户可见并可用。
     * @param packageName   要安装的现有应用的包名。
     * @param userId    目标用户的 ID（Android 支持多用户，如工作资料、访客等）
     * @param installFlags  安装标志，例如 INSTALL_INSTANT_APP（安装为 instant 应用）或 INSTALL_FULL_APP（安装为完整应用）
     * @param installReason 安装原因，例如 INSTALL_REASON_USER（用户主动安装）或 INSTALL_REASON_POLICY（策略安装）。
     * @param allowlistedRestrictedPermissions  一个可选的权限列表，表示在安装时需要授予的受限权限（通常用于企业设备策略）。
     * @param intentSender  一个 IntentSender，用于在安装完成后发送结果回调。该方法返回的 Pair 中也会携带此 IntentSender，以便调用方在同步流程结束后仍能获得回调对象。
     * @return 返回值是一个 Pair<Integer, IntentSender>，其中 Integer 表示安装结果码（如 INSTALL_SUCCEEDED 或各种错误码），IntentSender 是传入的或经过处理的回调对象
     */
    public Pair<Integer, IntentSender> installExistingPackageAsUser(@Nullable String packageName,
        ...
        /**
         * 调用者必须持有 INSTALL_PACKAGES 或 INSTALL_EXISTING_PACKAGES 权限，否则抛出 SecurityException。
         * INSTALL_EXISTING_PACKAGES 是一个专门用于安装现有包的权限，权限级别为 signature，通常只授予系统应用。
         */
        ...
        /**
         * 跨用户操作权限检查
         * 通过 preLockSnapshot.enforceCrossUserPermission(...) 确保调用者有权限在指定 userId 上执行操作，防止未经授权的用户间干扰。
         */
        ...
        /**
         * 用户限制检查
         * 如果目标用户被配置为禁止安装应用（通过 UserManager.DISALLOW_INSTALL_APPS 限制），则直接返回 INSTALL_FAILED_USER_RESTRICTED。
         */
        ...
        /**
         * 清除调用者身份标识
         * 使用 Binder.clearCallingIdentity() 暂时清除调用进程的身份，以便后续操作以系统身份执行（例如修改系统设置）。最后在 finally 块中恢复
         */
        ...
        try {
            ...
            synchronized (mPm.mLock) {
                final Computer snapshot = mPm.snapshotComputer();
                /**
                 * 获取包设置（PackageSetting）
                 * 通过 mPm.mSettings.getPackageLPr(packageName) 获取该包的系统内部数据结构 PackageSetting。
                 * 如果找不到或 PackageSetting 中的 pkg（即 PackageParser.Package 对象）为空，说明包不存在或解析失败，返回 INSTALL_FAILED_INVALID_URI。
                 */
                ...
                /**
                 * Instant 应用可见性处理
                 * 若调用者无权查看 instant 应用（通过 snapshot.canViewInstantApps 判断），则需要确保该包至少在一个用户上是以完整应用（full app）形式安装的。
                 * 遍历所有用户，只要有一个用户不是 instant 应用，就允许继续。否则返回 INSTALL_FAILED_INVALID_URI。
                 * 此检查防止普通应用绕过权限查看或操作只以 instant 形式存在的包
                 */
                ...
                /**
                 *  标记目标用户为已安装
                 *  如果目标用户尚未安装该包（!pkgSetting.getInstalled(userId)）：
                 */
                if (!pkgSetting.getInstalled(userId)) {
                    pkgSetting.setInstalled(true, userId);//将 installed 标志置为 true。
                    pkgSetting.setHidden(false, userId);//取消隐藏（setHidden(false)）。
                    pkgSetting.setInstallReason(installReason, userId);//设置安装原因 installReason。
                    pkgSetting.setUninstallReason(PackageManager.UNINSTALL_REASON_UNKNOWN, userId);
                    pkgSetting.setFirstInstallTime(System.currentTimeMillis(), userId);//记录首次安装时间（setFirstInstallTime）。
                    mPm.mSettings.writePackageRestrictionsLPr(userId);//调用 mPm.mSettings.writePackageRestrictionsLPr(userId) 将用户限制信息（如是否隐藏、是否暂停等）写入磁盘
                    mPm.mSettings.writeKernelMappingLPr(pkgSetting);//调用 mPm.mSettings.writeKernelMappingLPr(pkgSetting) 更新内核相关的映射（如 app-id 与用户的关系）
                    installed = true;
                } else if (fullApp && pkgSetting.getInstantApp(userId)) {
                    // upgrade app from instant to full; we don't allow app downgrade
                    //如果用户已经安装该包，但传入了 INSTALL_FULL_APP 标志且当前该用户下包是 instant 应用，则允许将 instant 应用升级为完整应用（installed 置为 true 并继续后续流程）。
                    //注意：不允许从完整应用降级为 instant 应用
                    installed = true;
                }

                //调用 ScanPackageUtils.setInstantAppForUser 根据 instantApp 和 fullApp 标志更新该用户下包的 instant/完整状态。
            ...
            if (installed) {
                /**
                 * 如果包有“更新所有者”（updateOwnerPackageName），且以下条件之一成立，则清空更新所有者：
                 * 调用者不是更新所有者本身（通过包名与调用者 UID 比对）。
                 * 该包不是系统应用，或者当前用户不是受管理的用户/资料（isFromManagedUserOrProfile）。
                 * 此逻辑确保在非受控环境中，更新所有者不会因安装现有包而意外残留。
                 */
                 ...
                 * 通过 mPm.mPermissionManager.onPackageInstalled 通知权限管理器该包已在目标用户上安装。
                 * 如果 installFlags 包含 INSTALL_ALL_WHITELIST_RESTRICTED_PERMISSIONS，
                 * 则会将包声明的所有权限作为“白名单受限权限”传递给权限管理器，以便在安装时授予这些权限（通常用于企业预配置）
                 */
                if (pkgSetting.getPkg() != null) {
                    ...
                    synchronized (mPm.mInstallLock) {
                        ...
                        /**
                         *  准备应用数据
                         * 在 mInstallLock 同步块中，调用 mAppDataHelper.prepareAppDataAfterInstallLIF
                         * 为该用户创建应用数据目录（如 /data/user/<userId>/<packageName>），并设置正确的 SELinux 标签和权限。
                         */
                ...
                /**
                 * 发送广播与更新序列号
                 * 调用 mPm.sendPackageAddedForUser 发送 ACTION_PACKAGE_ADDED 广播，通知系统其他组件该应用已对目标用户可用。
                 *
                 * 更新包的序列号（updateSequenceNumberLP），用于缓存失效等场景。
                 */
                ...
                /**
                 * 处理异步回调
                 * 如果原 intentSender 不为空，会将其封装到一个 InstallRequest 中，并在 restoreAndPostInstall 方法中启动异步操作。该异步操作会：
                 *
                 * 调用 mPm.restorePermissionsAndUpdateRolesForNewUserInstall 恢复该用户下与包相关的权限和角色（如默认应用角色）。
                 *
                 * 最后通过 onInstallComplete 将结果通过 IntentSender 回调给调用方。
                 *
                 * 同步方法返回时，如果 installed 为 true，返回的结果码为 INSTALL_SUCCEEDED；否则可能返回之前设置的错误码。
                 * 注意返回的 IntentSender 可能是经过处理的（如果原 intentSender 被用于异步回调，则返回的 intentSender 为 null）
                 */
    ...
    /**
     * onInstallComplete 是一个静态辅助方法，用于在包安装流程（如 installExistingPackageAsUser）完成后，通过 IntentSender 将安装结果异步回调给调用方
     * @param returnCode
     * @param context
     * @param target
     */
    ...
    /**
     * Android 包管理系统中连接安装与数据恢复的关键桥梁。它确保应用安装成功后，用户数据（无论是从备份恢复还是从回滚快照恢复）能正确到位，
     * 最后才通知系统其他组件应用已完全就绪。这种设计体现了 Android 在多用户、备份恢复、系统更新等复杂场景下对数据完整性和用户体验的考量。
     * @param request
     */
     ...
     *  在应用安装成功后通知备份管理器（Backup Manager）尝试从备份中恢复该应用的数据
     */
     ...
     * Returns whether the restore successfully completed.
     * 用于处理应用更新时的回滚（Rollback）数据恢复。它在 restoreAndPostInstall 中被调用，
     * 当安装操作是更新且没有执行备份恢复时，此方法会检查是否需要为回滚功能创建数据快照或恢复旧版本的数据。
     */
    ...
    /**
     * 多包安装的入口点，负责协调多个应用的批量安装
     * @param requests 一个 InstallRequest 对象列表，每个请求代表一个要安装的应用。
     *                 这些请求通常来自同一个安装会话（session），例如通过 PackageInstaller 提交的多个 APK（可能是同一个应用拆分的多个 APK，也可能是多个独立应用）。
     */
     ...
     * 负责实际执行批量安装的完整流程。它是 installPackagesTraced 的后续，包含了从准备、扫描、协调到提交的所有关键步骤。
     * 核心流程概览
     * 整个方法分为四个主要阶段：
     *
     * 准备与扫描阶段（持有 mInstallLock）
     *
     * 协调阶段（持有 mPackages 锁，即 mLock）
     *
     * 提交阶段（持有 mPackages 锁）
     *
     * 后置提交步骤（无锁）
        ...
        try {
            ...
            for (InstallRequest request : requests) {
                try {
                    ...
                    preparePackageLI(request); // 准备阶段：解析APK、验证签名等
                ...
                try {
                    ...
                    // 检查 PROPERTY_NO_APP_DATA_STORAGE 属性的一致性
            ...
            /**
             * mLock：保护内存包信息（mPackages）的锁。
             *
             * reconcilePackages：静态工具方法，核心协调逻辑：
             *
             * 检查签名一致性（更新时新老签名是否匹配）。
             *
             * 检查与现有包的冲突（如包名重复但签名不同）。
             *
             * 验证权限是否合规（如需要系统权限的应用是否签名正确）。
             *
             * 处理共享库依赖。
             *
             * 生成 ReconciledPackage 列表，包含最终要提交的包信息。
             *
             * 协调失败会导致整个批次失败。
             */
            ...
            synchronized (mPm.mLock) {
                ...
                try {
                    /**
                     * commitPackagesLocked：最终写入操作，在 mLock 保护下：
                     *
                     * 将 PackageSetting 添加到 mSettings 和 mPackages 内存映射。
                     *
                     * 更新共享库记录。
                     *
                     * 记录权限和组件信息。
                     *
                     * 调用 PermissionManagerService 通知权限变更。
                     *
                     * 此时内存中包已可见，但磁盘尚未完全同步（packages.xml 会在稍后写入）。
                     *
                     * 成功执行后设置 success = true，标志整个批次提交成功。
                     */
            ...
            /**
             * 执行 Dex 优化（dexopt）。
             * 准备应用数据目录（prepareAppDataAfterInstallLIF）。
             * 触发备份/回滚恢复（调用 restoreAndPostInstall 中已介绍的方法）。
             * 发送 ACTION_PACKAGE_ADDED/REPLACED 广播。
             * 写入 packages.xml 和 runtime-permissions.xml 持久化。
             */
    ...
    /**
     * 背景：PROPERTY_NO_APP_DATA_STORAGE
     * 这个属性是在 Android 10（API 29）引入的，定义在 PackageManager 类中：
     *
     * java
     * public static final String PROPERTY_NO_APP_DATA_STORAGE = "android.no_app_data_storage";
     * 它是一个布尔类型的应用级属性，在应用的 AndroidManifest.xml 中通过 <property> 标签声明。
     *
     * 当设置为 true 时，表示该应用不需要 CE（凭据加密）和 DE（设备加密）数据存储目录。
     *
     * 这类应用通常是纯代码执行、不存储用户数据的组件，如某些系统服务组件或预置的虚拟应用。
     * @param oldPkg
     * @param newPkg
     * @return
     */
    ...
    /**
     * 在安装锁（mInstallLock）保护下对单个包进行全面的安装前准备工作。它是 installPackagesLI 中第一个调用的关键方法，为后续的扫描、协调和提交阶段奠定基础。
     * @param request 封装了单个包安装所需的所有信息，包括安装标志、文件路径、用户等。
     * @throws PrepareFailure 抛出 PrepareFailure：如果准备过程中出现任何问题，抛出此异常，终止整个安装批次。
     * preparePackageLI 负责以下关键任务：
     *
     * 解析安装标志并生成扫描标志
     *
     * 解析APK文件生成 ParsedPackage
     *
     * 执行一系列安装前验证
     *
     * 处理包名重命名（如original packages）
     *
     * 签名验证和兼容性检查
     *
     * 权限和权限组冲突检测
    ...
    private void preparePackageLI(InstallRequest request) throws PrepareFailure {
        ...
        //1. 解析安装标志并生成扫描标志
        //根据 installFlags 设置对应的扫描标志，这些标志将指导后续的扫描行为。
        //例如：SCAN_AS_INSTANT_APP 表示该包应作为 instant 应用处理。
        ...
        //解析APK
        //使用 PackageParser2 解析 APK 文件，生成 ParsedPackage 对象。
        //parseFlags 包含 PARSE_CHATTY、PARSE_ENFORCE_CODE 等，控制解析行为。
        //验证 Dex 元数据完整性。
        ...
        //3. 目标SDK版本检查
        //MIN_INSTALLABLE_TARGET_SDK：当前系统要求的最低可安装目标SDK版本（Android 14 中为 23，即 Android 6.0）。
        //防止安装目标过旧的应用，这些应用可能缺乏现代安全特性。
        //可通过 INSTALL_BYPASS_LOW_TARGET_SDK_BLOCK 标志或 testOnly 属性绕过。
        ...
        //Instant 应用专项检查
        //Instant 应用必须 target O 及以上，不能声明 sharedUserId，必须使用 V2 及以上签名方案。
        if (instantApp) {
        ...
        //6. 签名信息设置
        //优先使用请求中预置的签名信息（如来自 PackageInstallerSession），否则从APK解析。
        ...
        //9. 系统应用更新检查
        //系统应用不能更新到外部存储，也不能更新为 instant 应用。
        if (systemApp) {
            if (onExternal) {
        ...
        if (request.isInstallMove()) {
            ...
            try {
                ...
                //10. ABI 推导
                //根据包的 native 库、系统类型和用户覆盖，推导出主次 ABI（如 arm64-v8a、armeabi-v7a）。
                //
                //结果设置到 parsedPackage 中，供后续使用。
        ...
        //11. 文件重命名和 FS-verity
        if (!isApex) {
            //doRenameLI：将临时 APK 文件重命名为最终名称（如 base.apk），并移动到最终位置。
            doRenameLI(request, parsedPackage);

            try {
                //setUpFsVerity：为 APK 文件设置 fs-verity，用于文件完整性验证（Android 14 新特性）。
        ...
        //12. 应用冻结
        //冻结应用进程，防止在更新过程中应用被启动或访问不一致的状态。
        //
        //冻结器（freezer）会在安装完成后由调用者负责关闭。
        ...
        try {
            ...
            //14. 设置准备结果
            //将所有准备阶段得出的信息打包到 InstallRequest 中，供后续扫描阶段使用。
    ...
    /**
     *
     *installPackagesFromDir 是系统应用扫描的最终执行者，负责将目录中的APK文件解析并注册到系统：
     * 参数详解：
     * 参数	类型	作用	示例值
     * scanDir	File	要扫描的目录	/system/priv-app
     * parseFlags	int	解析标志	PARSE_APK_IN_APEX
     * scanFlags	int	扫描标志	SCAN_AS_PRIVILEGED
     * packageParser	PackageParser2	解析器实例	用于解析APK
     * executorService	ExecutorService	线程池	并行处理
     * apexInfo	ActiveApexInfo	APEX信息	APEX模块数据
     *
     * @param scanDir
     * @param parseFlags
     * @param scanFlags
    ...
    public void installPackagesFromDir(File scanDir, int parseFlags,
        ...
        //
        //条件	说明	示例
        //isApkFile(file)	以.apk结尾的文件	Settings.apk
        //file.isDirectory()	可能是拆分APK目录	SplitAPKDir/
        //!isStageName()	不是临时安装目录	排除vmdl-*.tmp
        //为什么接受目录？
        //
        //支持拆分APK（Split APK）的安装格式
        //
        //目录内可能包含多个基础APK文件
        int fileCount = 0;
        //任务提交循环：
        ...
        //    participant Parser as PackageParser2
        //
        //    Main->>Queue: submit(file1)
        //    Main->>Queue: submit(file2)
        //    Main->>Queue: submit(file3)
        //
        //    Pool->>Queue: 获取任务
        //    Pool->>Parser: 解析file1
        //    Parser-->>Pool: 返回结果
        ...
        for (; fileCount > 0; fileCount--) {
            ...
            Throwable throwable = parseResult.throwable;
            // 处理三种情况：
            // 1. 解析成功 (throwable == null)
            // 2. 解析失败但已知异常 (PackageManagerException)
            // 3. 未知异常
            ...
            if (throwable == null) {
                ...
                //已知异常的处理
                //常见的 PackageManagerException 错误码：
                //
                //错误码	含义	可能原因
                //INSTALL_FAILED_INVALID_APK	APK无效	文件损坏
                //INSTALL_FAILED_DUPLICATE_PACKAGE	包重复	已存在相同包名
                //INSTALL_FAILED_UID_CHANGED	UID变化	签名不一致
                //INSTALL_PARSE_FAILED_MANIFEST_MALFORMED	Manifest错误	XML格式错误
                PackageManagerException e = (PackageManagerException) throwable;
                errorCode = e.error;
                errorMsg = "Failed to parse " + parseResult.scanFile + ": " + e.getMessage();
                Slog.w(TAG, errorMsg);
            } else {
                //为什么抛出 IllegalStateException？
                //不应该发生的异常
                //可能是代码bug或系统状态异常
                //强制终止，避免系统处于不一致状
                throw new IllegalStateException("Unexpected exception occurred while parsing "
     ...
     * 在系统初始化或安装过程中添加包的核心方法
     * 最终解析完成的 AndroidPackage 对象
     * 参数解析
     * 参数	类型	说明
     * parsedPackage	ParsedPackage	已解析的包信息（包含AndroidManifest数据）
     * parseFlags	int	解析标志位（如是否解析证书、组件等）
     * scanFlags	int	扫描标志位（如是否是系统应用、是否来自APEX）
     * user	UserHandle	目标用户（null表示所有用户）
     * activeApexInfo	ApexManager.ActiveApexInfo	APEX模块信息（如果是APEX安装）
     */
    @GuardedBy("mPm.mInstallLock")
    private AndroidPackage addForInitLI(ParsedPackage parsedPackage,
        ...
        //执行系统包扫描
        //ScanResult：扫描结果（包含PackageSetting）
        //Boolean：是否应该隐藏系统应用
        ...
        synchronized (mPm.mLock) {
            ...
            try {
                ...
                //这是Android 14中最重要的安全检查阶段，验证：
                //签名冲突：检查是否尝试用不同签名更新系统应用
                //权限兼容性：检查新版本是否请求了危险权限
                //共享库版本：检查引用的共享库是否存在且版本兼容
                //包名冲突：检查包名是否已被占用
                ...
                //将最终结果写入PackageManager的数据结构
                //mPackages、mSettings 等会被更新
